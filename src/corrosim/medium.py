...
class MediumSpec:
    """Parsed medium: the original label plus the chemistry we could extract.
...
def parse_medium(medium: str) -> MediumSpec:
    ...
    Never raises. Recognises an explicit ``pH <x>``, a ``<conc> M <acid>`` for
    common strong acids (strong-acid pH approximation), and a keyword fallback
    for everything else. Unrecognised media yield ``ph=None`` with an acidity
    guess from keywords.

    Args:
        medium: The free-text medium label (e.g. ``"1 M HCl"``).

    Returns:
        The parsed :class:`MediumSpec`.
    """
    label = medium.strip()
    low = label.lower()
