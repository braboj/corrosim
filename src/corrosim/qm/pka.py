...
G*_aq(H⁺) is the well-established aqueous proton free energy; the molecular
terms come from ddCOSMO single points.

APPROXIMATION (read before trusting an absolute number). With ``g_corr_*`` left
at 0 this uses the *electronic* aqueous energies only and omits the ZPE /
thermal / entropy of B and BH⁺ (no frequency calculation), and the geometries
...
# The widely used convention (Tissandier et al.); ~±2 kcal/mol spread.
...
def estimate_pka(e_neutral_aq_ev: float, e_cation_aq_ev: float,
    ...
    """Conjugate-acid pKaH via the deprotonation thermodynamic cycle.

    Uses the aqueous total energies of the neutral (B) and protonated cation
    (BH⁺). ``g_corr_*`` left at 0 gives the electronic-only estimate (see the
    module docstring on accuracy).

    Args:
        e_neutral_aq_ev: Aqueous (ddCOSMO) energy of the neutral B (eV).
        e_cation_aq_ev: Aqueous (ddCOSMO) energy of the cation BH⁺ (eV).
        g_corr_neutral_ev: ZPE+thermal+entropy correction for B (eV).
        g_corr_cation_ev: ZPE+thermal+entropy correction for BH⁺ (eV).
        temperature: Temperature (K).
        g_aq_proton_ev: Aqueous proton free energy (eV).

    Returns:
        The conjugate-acid pKaH.
    """
    g_b = e_neutral_aq_ev + g_corr_neutral_ev
    g_bh = e_cation_aq_ev + g_corr_cation_ev
    # ΔG*_aq for BH⁺ → B + H⁺
    dg_deprot_ev = g_b + g_aq_proton_ev - g_bh
