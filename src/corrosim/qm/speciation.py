...
The protonation equilibrium  B + H⁺ ⇌ BH⁺  is governed by the conjugate-acid
pKa (pKaH) of the inhibitor's most basic site. The protonated fraction follows
...
confident number. Treat the point pKaH as illustrative.
"""
...
def protonation_fraction(ph: float,
    ...
    Args:
        ph: The medium pH.
        pkah: The site's conjugate-acid pKaH.
...
def pkah_for_fraction(ph: float, f_prot: float) -> float:
    """Inverse of :func:`protonation_fraction`: the pKaH giving ``f_prot``.

    Useful for expressing a population crossover as a pKaH.

    Args:
        ph: The medium pH.
        f_prot: The target protonated fraction.

    Returns:
        The pKaH that yields ``f_prot`` at ``ph``.
    """
...
class Speciation:
    """Neutral/protonated population split at a pH (fractions sum to 1)."""
    ...
    def dominant(self) -> str:
        """Majority form at this pH.

        Returns:
            'protonated' or 'neutral' — whichever holds the majority.
        """
...
def speciate(ph: float, pkah: float = FLAVONOID_CARBONYL_PKAH) -> Speciation:
    ...
    Returns:
        The neutral/protonated :class:`Speciation` split.
    """
...
def blend_descriptors(neutral_row: dict, protonated_row: dict,
    ...
    'pH-weighted'. Fields missing (or non-numeric) in either row are left as
    the neutral value.

    Args:
        neutral_row: The neutral form's descriptor row.
        protonated_row: The protonated cation's descriptor row.
        f_protonated: The protonated-population weight in [0, 1].

    Returns:
        A new row with numeric descriptors blended by population.
    """
...
def _align(neutral_rows: list[dict],
           protonated_rows: list[dict]) -> list[tuple[dict, dict]]:
...
def analyse_speciation(neutral_rows: list[dict], protonated_rows: list[dict],
    ...
    Args:
        ...
        protonated_rows: The protonated-cation descriptor rows.
        ph: The medium pH.
        rank_fn: Ranks a list of descriptor rows best-first. Injected —
            typically ``report.rank_inhibitors`` wrapped for list I/O — to
            avoid a circular import.
        pkah: The site's conjugate-acid pKaH (the point estimate).
        band: The ± sensitivity band around ``pkah``.

    Returns:
        The population split at ``pkah``, the population-weighted
        ('pH-weighted') rows and their lead, the ``pkah ± band`` sensitivity
        range, and the protonation fraction at which the composite lead first
        changes (the crossover), as both a fraction and a pKaH.
    """
    ...
    blended_rows = [blend_descriptors(n, p, spec.f_protonated)
                    for n, p in pairs]
    ...
    crossover_pkah = (pkah_for_fraction(ph, crossover_f)
                      if crossover_f else None)
    return {
        "speciation": spec,
