...
| **Output** | The DFT-optimised geometry, persisted as `results/<case>/<molecule>_opt.xyz` (each neutral and its `+H+` cation; `run_dft --optimize`), plus the FF-vs-DFT robustness table `results/<case>/geometry_comparison.csv`. With `--check-minimum`/`--to-minimum`, each descriptor row also carries `n_imag` + `lowest_freq_cm`. (The descriptors *on* this geometry are written by the next step.) |
...
| **Output** | Split across the two subsections below (`results/<case>/dft_descriptors*.{csv,json}`, `results/<case>/<molecule>_fukui.json`, `cubes/`). |
...
`results/<case>/dft_descriptors_ff.{csv,json}` (the cheap FF geometry, the default
screen) and `results/<case>/dft_descriptors_opt.{csv,json}` (recomputed on the
...
**Output:** per-molecule `results/<case>/<molecule>_fukui.json` (Fukui); volumetric
...
| **Output** | `results/<case>/mc_adsorption.json`. |
...
| **Output** | `results/<case>/md_rdf.json`. |
...
| **Input** | The neutral global descriptors (`results/<case>/dft_descriptors_ff.{csv,json}`); E_ads and the metal–O distance joined in for display. |
| **Output** | The best-first ranking table in the report (`report/<case>/report.html`, `report.docx`). |
